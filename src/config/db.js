...
const create_sql_wrapper = async () => { // because of ECONNRESET | https://github.com/porsager/postgres/issues/179
   ...
   const sql_wrapper = async (strings, ...expressions) => { // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals#tagged_templates
      return await pass_by_reference.sql(strings, ...expressions) // FYI: the async and await can be omitted
   }
